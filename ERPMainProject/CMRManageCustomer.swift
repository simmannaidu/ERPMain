//
//  CMRManageCustomer.swift
//  ERPMainProject
//
//  Created by Hari on 31/05/20.
//  Copyright © 2020 Lancius. All rights reserved.
//

import UIKit
import Alamofire
class CMRManageCustomerTableViewCell : UITableViewCell
{
...
class CMRManageCustomer: UIViewController {
    @IBOutlet weak var CMRManageCustomerTableView: UITableView!
    ...
    override func viewWillAppear(_ animated: Bool) {
        if CheckInternet.Connection(){
           ...
           func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
               let cell = tableView .dequeueReusableCell(withIdentifier: " CMRManageCustomerTableViewCellId", for: indexPath) as! CMRManageCustomerTableViewCell
               /*
                   "dailing_id": "172",
                                ...
                                if response1?.statusCode == 200
                                {
                                     do{
                                       ...
                                       self.dataForTableView = items["crm_list"] as! [NSDictionary]
                                        ...
                                        {
                                            self.CMRManageCustomerTableView.isHidden = true
                                            Alert.showBasic(titte: "Sorry", massage: "No records found", vc: self)
                                        }
                                        else
                                        {
                                            self.CMRManageCustomerTableView.isHidden = false
                                        }
                                        self.CMRManageCustomerTableView.reloadData()
